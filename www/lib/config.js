$(function() {
  ...
  //懒加载
  $(document).on("pageInit", "#page-lazy", function(e, id, page) {
    $("div.lazy").lazyload({
      effect: "fadeIn",
      container: $("#lazyload")
    });
  ...
  //自动填充
  $(document).on("pageInit", "#autocomplete", function(e, id, page) {
    var remoteOptions = {
      limit: 2,
      datasource: 'remote',
    ...
    $.autocomplete(localOptions);
  });

  //进度条
  $(document).on("pageInit", "#progress", function(e, id, page) {
    $("#uploadFile").on("click", function(evt) {
      $("#progress-bar").show();
      var percent = 0;
      var percentCount = setInterval(function() {
        percent += 1;
  ...
  //获取设备信息
  $(document).on("pageInit", "#page-device", function(e, id, page) {
    console.log($.device);
    $("#device").html($.device.os + "--" + $.device.osVersion)
  ...
  //获取横竖屏信息
  $(document).on("pageInit", "#page-screen", function(e, id, page) {
  ...
  //计数器
  $(document).on("pageInit", "#page-countup", function(e, id, page) {
    $("#countup").countup({
      startVal: 0,
      endVal: 100,
      duration: 5
  ...
  //摇一摇
  $(document).on("pageInit", "#page-shake", function(e, id, page) {
    $.shake();
  });

  $(document).on("pageInit", "#page-star", function(e, id, page) {
    $('#star').raty({
      starHalf: 'icon-star_half',
      starOff: 'icon icon-star_blank',
      starOn: 'icon icon-star_fill',
      starType: 'i',
    });
    $('#heart').raty({
      starOff: 'icon icon-heart',
      starOn: 'icon icon-heart_fill',
      starType: 'i',
    });
  });
  ...
  $(document).on("pageInit", "#page-picker", function(e) {
